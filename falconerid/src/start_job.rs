...
/// The worker binary nearly fills the old 16 MiB memory limit of the init
/// container that copies it, so the copy intermittently got OOM-killed. Lock
/// the init container's resource shape down so that doesn't regress.
#[test]
fn copy_worker_has_enough_memory_to_copy_the_injected_binary() {
    use falconeri_common::serde_json;
    use serde_yaml;
    ...
    let init_containers = parsed["spec"]["template"]["spec"]["initContainers"]
        .as_array()
        .expect("initContainers should be an array");
    let copy_worker = init_containers
        .iter()
        .find(|container| container["name"] == "copy-worker")
        .expect("copy-worker init container should exist");

    assert_eq!(
        copy_worker["resources"]["requests"]["memory"].as_str(),
        Some("64Mi")
    );
    assert_eq!(
        copy_worker["resources"]["limits"]["memory"].as_str(),
        Some("64Mi")
    );
}
